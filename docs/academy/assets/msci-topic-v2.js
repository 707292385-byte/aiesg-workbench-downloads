(function () {
  ...
  function scoreSection(sections, renderLatex, esc) {
    const find = pattern => sections.findIndex(section => pattern.test(cleanTitle(section)));
    const rootIndex = find(/关键议题得分|总得分|最终得分/);
    const exposureIndex = find(/敞口得分$/);
    const managementIndex = find(/管理得分$/);
    if (rootIndex < 0 || exposureIndex < 0 || managementIndex < 0) return '';
    const root = sections[rootIndex];
    const formula = (root.blocks || []).find(block => block.type === 'formula');
    return `
      <section class="mp-section" id="score">
        <div class="mp-section-head"><div class="mp-section-title"><span>03</span><h2>评分结构</h2></div><p>关键议题得分由风险敞口和管理能力共同决定</p></div>
        <div class="mp-score-shell mp-card">
          <div class="mp-score-lead">
            ...
            <div class="mp-score-equation"><small>核心计算关系</small>${formula ? renderLatex(formula.content) : '关键议题得分 = 风险敞口与管理能力的综合结果'}</div>
          </div>
          <div class="mp-score-branches">
  ...
  function riskSection(sections, esc) {
    ...
    return `
      <section class="mp-section" id="risks">
        <div class="mp-section-head"><div class="mp-section-title"><span>04</span><h2>风险与应对关系</h2></div><p>风险来源、业务影响与管理响应放在同一视图</p></div>
  ...
  function frameworkSection(sections, groups, esc) {
    if (!groups.length) return '';
    return `
      <section class="mp-section" id="framework">
        <div class="mp-section-head"><div class="mp-section-title"><span>05</span><h2>管理评分框架</h2></div><p>先看父级维度，再进入各项指标定义</p></div>
  ...
  function formulaSection(sections, renderLatex, esc) {
    ...
    return `
      <section class="mp-section" id="formulas">
        <div class="mp-section-head"><div class="mp-section-title"><span>06</span><h2>关键计算方法</h2></div><p>先看计算关系，完整变量说明保留在核心内容中</p></div>
  ...
  function indicatorSection(sections, groups, renderBlock, esc) {
    if (!groups.length) return '';
    return `
      <section class="mp-section" id="indicators">
        <div class="mp-section-head"><div class="mp-section-title"><span>07</span><h2>指标体系与定义</h2></div><p>父级维度说明 + 指标卡片，按需展开详细定义</p></div>
  ...
  function sourceSection(detail, sections, sourceIndices, renderBlock, esc) {
    ...
    return `
      <section class="mp-section" id="sources">
        <div class="mp-section-head"><div class="mp-section-title"><span>08</span><h2>来源与使用说明</h2></div><p>正式工作请回到发布机构现行文件核对</p></div>
  ...
  window.renderMsciTopicV2 = function (api) {
    ...
    const overview = carbon ? [
      ...
      ['评价类型', detail.type || '关键议题', '结合风险敞口与管理表现'],
      ['内容范围', `${sections.length} 节`, `${stats.indicators} 项指标 · ${stats.formulas} 个公式`],
    ];
    const nav = [
      ['#overview', '概览'], ['#score', '评分结构'], ['#framework', '管理框架'], ['#indicators', '指标定义'], ['#sources', '来源'],
    ];
    return `
      <div class="msci-prototype-page">
        ...
        <section class="mp-hero" id="top"><div class="mp-hero-inner"><div class="mp-crumb">知识学堂　/　MSCI ESG评级方法论　/　${esc(detail.title)}</div><div class="mp-hero-grid"><div><span class="mp-eyebrow">关键议题 · ${esc(detail.titleEn || '')}</span><h1>${esc(detail.title)}</h1><p>${esc(detail.summary || '')}</p><div class="mp-hero-tags">${[detail.pillar, detail.theme, detail.type, '完整方法论', '中英对照'].filter(Boolean).map(value => `<span>${esc(value)}</span>`).join('')}</div></div><div class="mp-hero-side"><article><small>内容章节</small><strong>${sections.length}</strong></article><article><small>内容块</small><strong>${blockCount(sections)}</strong></article><article><small>公式</small><strong>${stats.formulas}</strong></article><article><small>管理维度</small><strong>${groups.length}</strong></article></div></div></div></section>
        <div class="mp-page">
          <section class="mp-section" id="overview"><div class="mp-section-head"><div class="mp-section-title"><span>01</span><h2>议题概览</h2></div><p>先判断适用范围，再进入评分和指标</p></div><div class="mp-overview-grid">${overview.map(item => `<article class="mp-metric mp-card"><small>${esc(item[0])}</small><strong>${esc(item[1])}</strong><span>${esc(item[2])}</span></article>`).join('')}</div><div class="mp-notice">阅读顺序：先看评分结构，再看管理框架，最后进入各指标定义与原文。</div></section>
          ...
          ${scoreSection(sections, renderLatex, esc)}
          ${riskSection(sections, esc)}
          ${frameworkSection(sections, groups, esc)}
          ${formulaSection(sections, renderLatex, esc)}
          ${indicatorSection(sections, groups, renderBlock, esc)}
          ${sourceSection(detail, sections, sourceIndices, renderBlock, esc)}
        </div>
